...
export const pedidos =  {
    ...
    script : ()=>{
        ...
        selectCerveza.addEventListener('change', () => {
            const imgSelect = bd.find(cerveza => cerveza.id == selectCerveza.value)

            if (imgSelect) {
                document.querySelector('#nombre').innerHTML = imgSelect.nombre
                document.querySelector('#descripcion').innerHTML = imgSelect.descripcion
                document.querySelector('#imagen').src = imgSelect.imagen
            }
        })
